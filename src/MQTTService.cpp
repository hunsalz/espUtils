...
MQTTService::~MQTTService() { end(); }

bool MQTTService::available() { return getMqttClient().connected(); }
...
bool MQTTService::begin(config_t config) {
  ...
  _mqttClient.onSubscribe(
      [](uint16_t packetId, uint8_t qos) { LOG.verbose(F("Subscribe acknowledged with packetId [%d]"), packetId); });
  _mqttClient.onUnsubscribe([](uint16_t packetId) { LOG.verbose(F("Unsubscribe acknowledged with packetId [%d]"), packetId); });
  _mqttClient.onMessage([](char *topic, char *payload, AsyncMqttClientMessageProperties properties, size_t len, size_t index,
                           size_t total) { LOG.verbose(F("Message received for topic [%s]: %s"), topic, payload); });
  _mqttClient.onPublish([](uint16_t packetId) { LOG.verbose(F("Publish acknowledged with packetId [%d]"), packetId); });
...
MQTTService::config_t MQTTService::getConfig() { return _config; }
...
AsyncMqttClient &MQTTService::getMqttClient() { return _mqttClient; }
